#!/usr/bin/env node
/**
 * Xero MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Xero secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Xero
 ...
 *      `Authorization: Bearer <xero-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Xero key, construct the client.
...
const SLUG = 'xero';
const NAME = 'Xero MCP Server';
const VERSION = '2.0.0';

/** Construct a XeroClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): XeroClient {
  const o = JSON.parse(credential); return new XeroClient(o.clientId, o.clientSecret, o.tenantId ?? process.env.XERO_TENANT_ID ?? "");
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'Xero Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Xero API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Xero key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <xero-api-key>',
    ],
  });
});
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Xero isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Xero is not connected for this caller yet. Connect it once via the link below (paste your Xero API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Xero is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-xero-api-key>' },
      });
      return;
    }
